...
class MLP(torch.nn.Module):
    ...
    def fit(self, dataset, steps=100, loss_fn=None, 
            lr=1., opt = "Adam", scheduler="exp", gamma=0.95, step_sch = 100):        
        nlog = 10
...
class Ensemble_MLP(torch.nn.Module):
    ...
    def fit(self, dataset, opt = "Adam", lr = 0.1, steps=100, scheduler="exp", gamma=0.95, step_sch = 100):
...
class Ensemble_KAN(torch.nn.Module):
    ...
    def fit(self, dataset, opt = "Adam", lr = 0.1, steps=100, scheduler="exp", gamma=0.95, step_sch = 100,
            ...
            loss = [self.modelkan[i].fit(dataset, opt = opt, steps = steps, 
                    ...
                    loss_fn=loss_fn )  for i in range(self.nens)]
            return loss               
    def saveckpt(self, path='model'):
        dic = dict(
            width            = self.width           , 
