...
def B_batch(x, grid, k=0, extend=True, device='cpu'):
    ...
    #     diff_lt = x - grid[:, :, 1:]
    #     with torch.no_grad():
    #         ge = torch.heaviside(diff_ge, torch.tensor(1.0, dtype=diff_ge.dtype, device=diff_ge.device))
    #         lt = 1.0 - torch.heaviside(diff_lt, torch.tensor(1.0, dtype=diff_lt.dtype, device=diff_lt.device))
    #     value = ge * lt
    # else:
    #     B_km1 = B_batch(x[:,:,0], grid=grid[0], k=k - 1)

    #     value = (x - grid[:, :, :-(k + 1)]) / (grid[:, :, k:-1] - grid[:, :, :-(k + 1)] + eps) * B_km1[:, :, :-1] + (
    #                 grid[:, :, k + 1:] - x) / (grid[:, :, k + 1:] - grid[:, :, 1:(-k)] + eps) * B_km1[:, :, 1:]
...
def coef2curve(x_eval, grid, coef, k, device="cpu"):
    ...
    y_eval = (b_splines.unsqueeze(-2) * coef.to(b_splines.device).unsqueeze(0)).sum(-1)
