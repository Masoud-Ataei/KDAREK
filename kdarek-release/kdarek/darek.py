...
def divided_differences2(x, y):
    ...
    for j in range(1, n):
        ...
        #     breakpoint()
        coef[:, :n - j, j] = torch.nan_to_num(coef[:, :n-j, j], nan=0.0, posinf=0.0, neginf=0.0)
    return coef[:, 0, :]
...
def select_knots(x, g, kp, method = 'topk'):
    ...
    if method == 'topk':        
        ...
        diff = torch.abs(g-x)
        index_knots = torch.sort(torch.topk(diff, kp, 1, False)[1], dim = 1)[0]

        ### beginig index
        knots = g.T[index_knots][...,0]
        index_begining = (~torch.logical_or(knots[:,:1] < x, index_knots[:,:1] == 0))[:,0]
        index_knots[index_begining] -= 1
        knots = g.T[index_knots][...,0]

        ### end index
        index_end = (~torch.logical_or(knots[:,-1:] > x, index_knots[:,-1:] == (g.shape[1]-1)))[:,0]
...
#     return int_err, util
def newton_polynomial(x, y, x_eval):
    """Evaluate Newton's interpolating polynomial.  x,y : (batch,n)  x_eval : (batch,)"""
    coef   = divided_differences2(x, y)
    n      = coef.shape[1]
    result = coef[:, -1]
    for i in range(n - 2, -1, -1):
    ...
    return result
# ---------------------------------------------------------------------------
# OPTIMISATION 1 + 2 : vectorised spline_error
# ---------------------------------------------------------------------------

def spline_error(g, e, x, kp=4, Lkp=1.0, knot_select='topk', oint=None, oknot=None):
    """
    Vectorised over the output dimension (no Python loop over o).

    Parameters
    ----------
    g      : (n_grid,)
    e      : (n_knots, n_model_outputs)   – errors at knot grid points
    x      : (n_samples,)                 – evaluation points
    Returns
    -------
    int_err : (n_samples,)
    util    : (n_model_outputs, n_samples)
    """
        

    # ── interpolation error bound ──────────────────────────────────────────
    fac = math.factorial(oint)
    knots_int, _ = select_knots(x.view(-1, 1), g.view(1, -1), kp=oint, method=knot_select)
    int_err = torch.abs(torch.prod(x.view(-1, 1) - knots_int, dim=1)) * Lkp / fac

    # ── knot utility ───────────────────────────────────────────────────────    
    
    _, index_knots = select_knots(x.view(-1, 1), g.view(1, -1), kp=oknot, method=knot_select)
    x_segment = g.flatten()[index_knots]          # (n_samples, oknot)

    n_out     = e.shape[1]
    n_samples = x.shape[0]
    gmin, gmax = g.min(), g.max()
    interp_mask = (x >= gmin) & (x <= gmax)       # (n_samples,)

    # --- interpolation part (vectorised over n_out) -----------------------
    # e[index_knots] → (n_samples, oknot, n_out)
    y_seg = torch.abs(e[index_knots])              # (n_samples, oknot, n_out)
    # reshape to batch all outputs through newton_polynomial at once
    y_seg_flat   = y_seg.permute(2, 0, 1).reshape(n_out * n_samples, oknot)
    x_seg_flat   = x_segment.unsqueeze(0).expand(n_out, -1, -1).reshape(n_out * n_samples, oknot)
    x_eval_flat  = x.unsqueeze(0).expand(n_out, -1).reshape(n_out * n_samples)

    yhat_inter = torch.abs(newton_polynomial(x_seg_flat, y_seg_flat, x_eval_flat))
    yhat_inter = yhat_inter.reshape(n_out, n_samples)   # (n_out, n_samples)

    # --- extrapolation part (vectorised) -----------------------------------
    e_abs   = torch.abs(e)                              # (n_knots, n_out)
    # broadcast: (n_out, 1) + (1, n_samples) → (n_out, n_samples)
    e_left  = torch.clamp(e_abs[0].unsqueeze(1)  + (g[0]  - x).unsqueeze(0), 0.0, 1.0)
    e_right = torch.clamp(e_abs[-1].unsqueeze(1) + (x - g[-1]).unsqueeze(0), 0.0, 1.0)
    yhat_extra = torch.where(x.unsqueeze(0) < gmin, e_left, e_right)

    # --- combine -----------------------------------------------------------
    util = torch.where(interp_mask.unsqueeze(0), yhat_inter, yhat_extra)  # (n_out, n_samples)
    return int_err, util
...
class DAREK(MultKAN):
    ...
    def forward_update_grid(self, x, y, singularity_avoiding=False, y_th=10., reindex =False, seed = 0, method = 'random', index = None):    
        ...
        with torch.no_grad():
            for l in range(self.depth):
                ...
                if self.extend:
                    grid0, indx0 = grid.sort(dim=0)         # (n_pts, n_dim)
                    n_pts, n_dim = grid0.shape
                    # ── OPTIMISATION 2: vectorised extend block ───────────────
                    # Original had:  for i in range(n_dim): ...
                    dg  = (grid0[-1] - grid0[0]) / n_dim    # (n_dim,) - preserve original divisor
                    gz  = torch.empty(n_pts + 2 * k, n_dim,
                                    dtype=grid0.dtype, device=grid0.device)
                    gz[k:-k] = grid0                        # no .clone() on the source
                    al = torch.arange(-k, 0,   dtype=grid0.dtype, device=grid0.device).unsqueeze(1)  # (k,1)
                    ar = torch.arange(1, k+1,  dtype=grid0.dtype, device=grid0.device).unsqueeze(1)
                    gz[:k]  = al * dg + grid0[0]            # broadcasts over n_dim
                    gz[-k:] = ar * dg + grid0[-1]
                    grid0 = gz
                else:
                    grid0,indx0 =grid.sort(dim=0)              
    ...
    def predict(self, x0, fk = 1.0, f1 = 1.0, share = None,
                    knot_select = 'nearGj', noise = 0.0, oint = None, oknot = None, classify = False):
        '''
        umode : 'darek' or 'drain' or 'both'
        '''        
        self.splines = {'samples': self.samples}
        depth = len(self.width) - 1
        gout = self.knots[f'x{self.depth}'] #self(self.samples['x'])       
        ...
        k  = self.k
        kp = k + 1
        oint  = kp if oint  is None else oint
        oknot = kp if oknot is None else oknot

        disubar = oint  <= 0;  oint  = max(oint,  1)
        disutil = oknot <= 0;  oknot = max(oknot, 1)
        ...
        mlprod = float(np.sum(ws[:-1] * ws[1:]))
        if np.isscalar(fk):
            fk = [float(np.power(fk/ mlprod,1/depth))] * depth
        if np.isscalar(f1):
            f1 = [float(np.power(f1/ mlprod,1/depth))] * depth

        if share is None:
            s = sum(
                self.width_in[l] * self.width_out[l+1] * (f1[l] if l < depth-1 else 1.0) 
                for l in range(depth))
            share = 1.0 / s
            # share = 1.0
        
        if np.isscalar(share):
            share = [float(share)] * depth

        ubar_store = [{} for _ in range(depth)]
        util_store = [{} for _ in range(depth)]

        for l in range(depth):
            n_in = self.width_in[l]

            # ── OPTIMISATION 3: layer-level propagation sum ──────────────────
            #    Same for every i in this layer – compute once.
            if l > 0:
                prev_ubar_sum = torch.stack([ubar_store[l - 1][b] for b in range(self.width_in[l - 1])]).sum(0)   # (n_test,)
                prev_util_sum = torch.stack([util_store[l - 1][b] for b in range(self.width_in[l - 1])]).sum(0)   # (n_model_out, n_test)

            # ── OPTIMISATION 4: DRAIN distance matrix once per layer ─────────
            

            # ── OPTIMISATION 1+2: inner loop over i only (j removed) ─────────
            for i in range(n_in):
                indx    = self.knots[f'indx{l}'][:, i]
                ej_i    = ej[indx] * share[l]
                gi      = self.act_fun[l].grid[i]
                hx      = self.acts[l][:, i]
                gi_flat = gi.flatten()[k:-k] if self.extend else gi.flatten()

                ubar_c, util_c = spline_error(gi_flat, ej_i, hx, kp, fk[l],
                                            knot_select=knot_select,
                                            oint=oint, oknot=oknot)

                if disubar:
                    ubar_c = torch.zeros_like(ubar_c)
                if disutil:
                    util_c = torch.zeros_like(util_c)

                # Propagate
                if l > 0:
                    ubar_store[l][i] = ubar_c + f1[l] * prev_ubar_sum
                    util_store[l][i] = util_c + f1[l] * prev_util_sum
                else:
                    ubar_store[l][i] = ubar_c
                    util_store[l][i] = util_c

        # ── Final aggregation ──────────────────────────────────────────────────
        last_n_in  = self.width_in[depth - 1]
        n_outputs  = self.width_out[depth]

        # Sum over last-layer inputs
        ubar_final = torch.stack([ubar_store[depth - 1][b] for b in range(last_n_in)]).sum(0)
        util_sum   = torch.stack([util_store[depth - 1][b] for b in range(last_n_in)]).sum(0)
        # (n_model_outputs, n_test)

        # Preserve original semantics: Et accumulates n_outputs times
        Et = util_sum * n_outputs   # (n_model_outputs, n_test)

        # Build output tensor and populate self.splines for downstream compat
        U_list = [Et[i].flatten() + ubar_final.flatten() for i in range(n_outputs)]
        # for i in range(n_outputs):
        #     u_i = Et[i].flatten() + ubar_final.flatten()   # (n_test,)
        #     U_list.append(u_i)
            # self.splines[f'U{i}'] = {
            #     'Ubar': ubar_final,
            #     'Util': Et[i].flatten(),
        ...
        # Also rebuild per-edge spline entries so ErrL / plot_err still work
        # (fast because we just copy already-computed tensors)
        # for l in range(depth):
        #     for i in range(self.width_in[l]):
        #         for j in range(self.width_out[l + 1]):
        #             self.splines[f'{l}-{i}-{j}'] = {
        #                 'Ubar': ubar_store[l][i],
        #                 'Util': util_store[l][i],
        #             }

        return yhat, torch.stack(U_list, dim=1)   # (n_test, n_outputs)
